"""Run the frozen VisDrone 640-to-1280 detector fine-tuning pilot."""
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if config["dataset"]["selection_split"] != "val":
        raise ValueError("high-resolution pilot selection must use val")
...
def run_training(config_path: Path, *, smoke: bool) -> int:
    ...
    record: dict[str, Any] = {
        ...
        "claim_boundary": (
            ...
            if smoke
            else "Validation-selected high-resolution fine-tuning pilot; no test data used."
        ),
        "evidence": evidence,
